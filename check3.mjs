...
async function run() {
    ...
    console.log(data.filter(m => m.home_team.toLowerCase().includes('m') && m.away_team.toLowerCase().includes('sud')));
    // Just find "Sud"
    console.log("Sud:", data.filter(m => m.home_team.toLowerCase().includes('sud') || m.away_team.toLowerCase().includes('sud')));
}
